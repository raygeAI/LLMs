...
def generate_prompt(
    ...
    context = "\n".join([doc.page_instruction for doc in related_docs])
...
def search_result2docs(search_results):
    ...
    for result in search_results:
        doc = Document(page_instruction=result["snippet"] if "snippet" in result.keys() else "",
...
class LocalDocQA:
    ...
    def one_knowledge_add(self, vs_path, one_title, one_conent, one_instruction_segmentation, sentence_size):
        try:
            if not vs_path or not one_title or not one_conent:
                logger.info("知识库添加错误，请确认知识库名字、标题、内容是否正确！")
                return None, [one_title]
            docs = [Document(page_instruction=one_conent + "\n", metadata={"source": one_title})]
            if not one_instruction_segmentation:
    ...
    def get_knowledge_based_answer(self, query, vs_path, chat_history=[], streaming: bool = STREAMING):
        ...
        vector_store.chunk_conent = self.chunk_instruction
    ...
    def get_knowledge_based_instruction_test(self, query, vs_path, chunk_conent,
        ...
        if not related_docs_with_score:
            ...
            return response, ""
        torch_gc()
        prompt = "\n".join([doc.page_instruction for doc in related_docs_with_score])
        response = {"query": query,
                    "source_documents": related_docs_with_score}
