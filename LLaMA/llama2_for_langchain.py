...
class Llama2(LLM):
    ...
    def __init__(self, model_name_or_path, bit4=True):
        ...
        if not bit4:
            ...
            self.model = AutoModelForCausalLM.from_pretrained(model_name_or_path, device="cuda:0",
                                                              torch_dtype=torch.float16, load_in_8bit=True)
            self.model.eval()
        else:
            from auto_gptq import AutoGPTQForCausalLM
            self.model = AutoGPTQForCausalLM.from_quantized(model_name_or_path, low_cpu_mem_usage=False, device="cuda:0",
